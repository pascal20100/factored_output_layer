...
#include "CudaSparseTargetFactoredLinearOutputLayer.h"


PP<CudaSparseTargetFactoredLinearOutputLayer> largesparse_gpu_get_layer(int d, int D, int K, int m, 
...
{
  static PP<CudaSparseTargetFactoredLinearOutputLayer> layer = 0;

  if (layer==0)
    {
      layer = new CudaSparseTargetFactoredLinearOutputLayer();
    }

  layer->d = d;
  layer->D = D;

  layer->U = CublasMat<real>(U_data, d, d, U_stride);
  layer->UinvT = CublasMat<real>(UinvT_data, d, d, UinvT_stride);
  layer->VT = CublasMat<real>(V_data, d, D, V_stride);

  // Use Q or Q_tilde ?
  layer->use_Qtilde = use_Qtilde;
  if (use_Qtilde)
    layer->Qtilde = CublasMat<real>(Q_data, d, d, Q_stride);  
  else
    layer->Q = CublasMat<real>(Q_data, d, d, Q_stride);  
...
void largesparse_gpu_maintainance(int maintainance_op,
                                  int d, int D, 
                                  float* V_data, int V_stride,
                                  float* U_data, int U_stride, 
                                  float* UinvT_data, int UinvT_stride,
...
{
  PP<CudaSparseTargetFactoredLinearOutputLayer> layer 
    = largesparse_gpu_get_layer(
                                d, D, -1, -1, 
                                V_data, V_stride,
                                U_data, U_stride, 
...
void largesparse_gpu_fbprop_update(int d, int D, int K, int m, 
                                  float* V_data, int V_stride,
                                  float* U_data, int U_stride, 
                                  float* UinvT_data, int UinvT_stride,
                                  float* Q_data, int Q_stride,
...
{
  PP<CudaSparseTargetFactoredLinearOutputLayer> layer 
    = largesparse_gpu_get_layer(d, D, K, m, 
                                V_data, V_stride,
                                U_data, U_stride, 
                                UinvT_data, UinvT_stride,
                                Q_data, Q_stride,
...
//    }
  ...
  CublasMat<real> H(H_data, d, m, H_stride);

  CublasMat<real> grad_H(grad_H_data, grad_H_data!=0 ?d :0, grad_H_data!=0 ?m :0, grad_H_stride);
